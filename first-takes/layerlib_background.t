...
terra BackgroundPattern:free(type: enum, lib: &Lib)
	self.pattern:free()
	self.pattern = nil
	...
	if self.transform_id ~= 0 then
		lib.transforms:release(self.transform_id)
		self.transform_id = 0
	end
end

terra Background:free(lib: &Lib)
	if self.type > BACKGROUND_TYPE_COLOR then
		self.pattern:free(self.type, lib)
	end
end
...
terra Layer:paint_background(b: &Background, cr: &cairo_t)
	...
	self.lib.transforms:at(b.pattern.transform_id):apply(&m)
	m:invert()
	var patt = b:pattern()
	patt:matrix(&m)
