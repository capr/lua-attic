...
terra Window:sync(w: num, h: num)
	var view = self.view
	if view.layout_solver.sync_top(view, w, h) then
		view:sync_layout()
	end
end
...
local function align_items_main_axis_func(items_T, GET_ITEM, T, W)
	local _MIN_W = '_min_'..W
	return terra(
...
local function gen_funcs(X, Y, W, H)
	...
	local align_items_main_axis_x = align_items_main_axis_func(Layer, 'child', Layer, W)
	...
	local terra linewrap_next(self: &Layer, i: int): {int, int}
		...
		elseif not self.flex.wrap then
			return self.child_count-1, i
		end
		var wrap_w = self.[CW]
		var line_w = 0
		for j = i, self.child_count do
			var layer = self:child(j)
			if layer.visible then
	...
	Layer.methods['flex_min_cw_'..X] = terra(
		...
		if self.flex.wrap then
			return items_max_x(self, 0, self.child_count)._0
		else
			return items_sum_x(self, 0, self.child_count)._0
		end
	end
	...
	local terra stretch_items_x(self: &Layer, i: int, j: int, moving: bool)
		stretch_items_main_axis_x(
			self, i, j, self.[CW], self.flex.[ITEM_ALIGN_X], moving)
...
local align_cols_main_axis = align_items_main_axis_func(arr(GridLayoutCol), 'at', GridLayoutCol, 'w')
