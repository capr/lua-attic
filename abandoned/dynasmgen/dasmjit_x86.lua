...
function wrap(name, f)
	local args
	local function pass(...)
		local ret = select('#', ...) > 0 and '-> '..fmtargs(...) or ''
		print(string.format('  %s%s', ('  '):rep(lvl), ret))
		lvl = lvl - 1
		return ...
	end
	return function(...)
		lvl = lvl + 1
		print(string.format('  %s%s(%s)', ('  '):rep(lvl), name, fmtargs(...)))
		return pass(f(...))
	end
end

-- Inherited tables and callbacks.
local wline, werror, wwarn
...
if x64 then
	mkrmap("q", {"rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi"})
end
...
-- Put action for label arg (IMM_LG, REL_LG).
...
local function wputmrmsib(t, imark, s)
	...
	if tdisp == "number" then -- Check displacement size at assembly time.
		if disp == 0 and band(reg, 7) ~= 5 then -- [ebp] -> [ebp+0] (in SIB, too)
			m = 0 -- Force DISP to allow [Rd(5)] -> [ebp+0]
	...
	if xreg or band(reg, 7) == 4 then
		...
		if (imark == "I" and (m == 1 or m == 2)) then waction("MARK") end
	end
...
local function rtexpr(expr) --returns reg, regnum, typedef
	if not expr then return end
	local tname, treg = match(expr, "^([%w_]+):(@[%w_]+)$") --try `type:reg` syntax
	local tp = map_type[tname or expr]
	if tp then
		local reg = treg or tp.reg
	...
	return expr, map_reg_num[expr] --reg, rnum
end
...
--   "I"       ditto, but generates an action code to optionally modify
...
map_op[".label_1"] = function(params)
	if not params then return "[1-9] | ->global" end
	local a = parseoperand(params[1])
	local mode, imm = a.mode, a.imm
...
immexpr = wrap('immexpr', immexpr)
dispexpr = wrap('dispexpr', dispexpr)
rtexpr = wrap('rtexpr', rtexpr)
dopattern = wrap('dopattern', dopattern)
parseoperand = wrap('parseoperand', parseoperand)
waction = wrap('waction', waction)
...
function _M.dumparch(out)
	dumpregs(out or io.stdout)
end

-- Dump all user defined elements.
function _M.dumpdef(out, lvl)
	dumptypes(out or io.stdout)
end
...
function _M.passcb(wl, we, wf, ww)
	wline, werror, wwarn = wl, we, ww
	return wflush, actlist
end
